...
class AuthenticationManager:
    ...
    def authenticate(
        ...
        # Verify password
        if not self.encryption.verify_password(password, user["password_hash"], user["salt"]):
    ...
    def change_password(self, username: str, old_password: str, new_password: str) -> bool:
        ...
        # Verify old password
        if not self.encryption.verify_password(old_password, user["password_hash"], user["salt"]):
            return False
...
class AuditLogger:
    ...
    def log_login(self, username: str, success: bool, ip_address: Optional[str] = None):
        """Log login attempt"""
        self.log_event("login", username, {"success": success}, ip_address)

    def log_logout(self, username: str, ip_address: Optional[str] = None):
        """Log logout"""
        self.log_event("logout", username, {}, ip_address)

    def log_data_access(
        self, username: str, resource: str, action: str, ip_address: Optional[str] = None
    ):
        """Log data access"""
        self.log_event(
            "data_access", username, {"resource": resource, "action": action}, ip_address
        )

    def log_permission_change(self, username: str, target_user: str, old_role: str, new_role: str):
        """Log permission change"""
        self.log_event(
            "permission_change",
            username,
            {"target_user": target_user, "old_role": old_role, "new_role": new_role},
        )

    def log_security_event(self, username: str, event_description: str, severity: str = "medium"):
        """Log general security event"""
        self.log_event(
            "security_event", username, {"description": event_description, "severity": severity}
        )
