    ...
    def validate(password: str) -> tuple[bool, List[str]]:
        ...
        if SecurityConfig.PASSWORD_REQUIRE_SPECIAL:
            if not any(c in SecurityConfig.PASSWORD_SPECIAL_CHARS for c in password):
                errors.append(
                    f"Password must contain at least one special character: {SecurityConfig.PASSWORD_SPECIAL_CHARS}"
                )
...
class AuditLogger:
    ...
    def log_security_event(self, user: str, action: str, details: dict):
        ...
        self.log_event(
            event_type="security", user=user, action=action, result="success", details=details
        )
...
def check_security_configuration():
    ...
    if SecurityConfig.JWT_SECRET_KEY == "CHANGE_THIS_IN_PRODUCTION":
        warnings.append(
            "⚠️  WARNING: Using default JWT secret key. Set XENO_JWT_SECRET environment variable!"
        )
