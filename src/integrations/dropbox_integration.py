...
class DropboxIntegration(IntegrationBase):
    ...
    def __init__(self, credentials: Optional[IntegrationCredentials] = None):
        ...
        self.access_token = (
            credentials.api_key if credentials else os.getenv("DROPBOX_ACCESS_TOKEN")
        )
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.api_url}/users/get_current_account", headers=self.headers
                ) as response:
    ...
    async def upload_file(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.content_url}/files/upload", headers=headers, data=file_data
            ) as response:
