...
class JiraIntegration(IntegrationBase):
    ...
    def __init__(self, credentials: Optional[IntegrationCredentials] = None):
        ...
        self.domain = credentials.additional_data.get("domain") if credentials else os.getenv("JIRA_DOMAIN")
        self.email = credentials.additional_data.get("email") if credentials else os.getenv("JIRA_EMAIL")
