...
class GoogleDriveIntegration(IntegrationBase):
    ...
    async def upload_file(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                f"{self.upload_url}/files?uploadType=multipart", headers=headers, data=body
            ) as response:
    ...
    async def create_folder(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.base_url}/files", headers=self.headers, json=metadata
            ) as response:
                result = await response.json()
                return {
                    "success": response.status == 200,
    ...
    async def move_file(
        self, file_id: str, new_parent_id: str, current_parent_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.patch(
                f"{self.base_url}/files/{file_id}", headers=self.headers, params=params
            ) as response:
                return {"success": response.status == 200}

    async def copy_file(
        self, file_id: str, new_name: Optional[str] = None, parent_folder_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """Copy a file"""
        metadata = {}
        if new_name:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.base_url}/files/{file_id}/copy", headers=self.headers, json=metadata
            ) as response:
                result = await response.json()
                return {
                    "success": response.status == 200,
    ...
    async def share_file(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                result = await response.json()
                return {"success": response.status == 200, "permission_id": result.get("id")}

    async def delete_file(self, file_id: str) -> Dict[str, Any]:
        """Delete a file"""
        async with aiohttp.ClientSession() as session:
            async with session.delete(
                f"{self.base_url}/files/{file_id}", headers=self.headers
            ) as response:
    ...
    async def download_file(self, file_id: str, destination_path: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/files/{file_id}?alt=media", headers=self.headers
            ) as response:
    ...
    async def search_files(self, query: str, max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/files", headers=self.headers, params=params
            ) as response:
                if response.status == 200:
                    result = await response.json()
    ...
    async def list_files(
        self, folder_id: Optional[str] = None, max_results: int = 100
    ) -> List[Dict[str, Any]]:
