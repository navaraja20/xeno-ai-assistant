...
class IntegrationRegistry:
    ...
    def get_integration(
        self, service_name: str, credentials: Optional[IntegrationCredentials] = None
    ) -> IntegrationBase:
...
class WorkflowEngine:
    ...
    async def execute_workflow(
        ...
        try:
            ...
            for action in workflow.actions:
                # Check conditions
                if action.conditions and not self._evaluate_conditions(
                    action.conditions, execution_context
                ):
                ...
                results.append({"action_id": action.action_id, "success": True, "result": result})
    ...
    def _log_execution(
        self, workflow: Workflow, success: bool, results: List[Dict], error: Optional[str] = None
    ):
