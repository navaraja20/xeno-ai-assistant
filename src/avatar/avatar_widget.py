...
from src.avatar.avatar_engine import AvatarPose, EmotionState, get_avatar_engine
...
class AvatarWidget(QWidget):
    ...
    def _render_avatar_simple(self, painter: QPainter, rect: QRectF, avatar_data: dict):
        ...
        body_y = head_y + 140
        self.holo_renderer.render_glow(
            painter, QRectF(center_x - 80, body_y, 160, 200), intensity=0.5
        )
        ...
        painter.drawText(
            int(rect.left() + 10), int(rect.bottom() - 10), f"Emotion: {emotion.title()}"
        )
