        ...
        if self._driver is None:
            try:
                ...
                chrome_options.add_argument(
                    "--start-minimized"
                )  # Start minimized instead of headless
                ...
                chromedriver_path = os.path.expandvars(
                    r"%USERPROFILE%\.wdm\drivers\chromedriver\win64\142.0.7444.175\chromedriver-win64\chromedriver.exe"
                )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for job_data in all_jobs:
                    ...
                    # Filter by keywords
                    if any(
                        kw.lower() in title or kw.lower() in description or kw.lower() in category
                        for kw in keywords
                    ):
                        ...
                        if any(
                            ...
                            if not any(
                                kw in location_lower for kw in ["france", "paris", "french", "fr "]
                            ):
                                continue
                        ...
                        job = JobOpportunity(
                            ...
                            job_type="internship"
                            if is_internship
                            else job_data.get("job_type", "full_time"),
                            description=job_data.get("description", "")[:500],
                            requirements="",
                            posted_date=job_data.get(
    ...
    def scrape_themuse(
        ...
        try:
            ...
            for keyword in keywords:
                params = {"category": keyword, "level": "Internship", "page": 0}
                ...
                if response.status_code == 200:
                    ...
                    for item in results[:max_results]:
                        try:
                            ...
                            job_location = (
                                locations[0].get("name", location) if locations else location
                            )
                    ...
                    for item in items:
                        try:
                            ...
                            job = JobOpportunity(
                                ...
                                location=location,
                                job_type="internship"
                                if any(kw in title.lower() for kw in ["intern", "stage"])
                                else "full-time",
    ...
    def scrape_welcometothejungle(
        ...
        try:
            for keyword in keywords:
                ...
                job_cards = (
                    soup.find_all("li", attrs={"data-testid": "job-list-item"})
                    or soup.find_all(
                        "div", class_=lambda x: x and "job" in x.lower() and "card" in x.lower()
                    )
                    or soup.find_all(
                        "a", href=lambda x: x and "/jobs/" in x and "/fr/companies/" in x
                    )
                )
                ...
                for card in job_cards[:max_results]:
                    try:
                        ...
                        if card.name == "a":
                            ...
                            job_url = (
                                f"https://www.welcometothejungle.com{card['href']}"
                            ...
                            company_elem = (
                                parent.find("span", string=lambda x: x and len(x) > 2)
                                if parent
                                else None
                            )
                            company = (
                            ...
                            job_location = (
                                location_elem.get_text(strip=True) if location_elem else "France"
                            )
                        else:
                            # Card element
                            ...
                            company_elem = card.find(
                                "span", attrs={"data-testid": "job-company-name"}
                            ) or card.find(
                                string=lambda x: len(str(x)) > 3 and str(x).isupper() == False
                            )
                            company = (
                                company_elem.get_text(strip=True)
                                if hasattr(company_elem, "get_text")
                                else str(company_elem).strip()
                                if company_elem
                                else "Unknown"
                            )

                            location_elem = card.find(
                                "span", attrs={"data-testid": "job-location"}
                            ) or card.find(string=lambda x: "Paris" in str(x) or "France" in str(x))
                            job_location = (
                                location_elem.get_text(strip=True)
                                if hasattr(location_elem, "get_text")
                                else str(location_elem).strip()
                                if location_elem
                                else "France"
                            )
            ...
            for keyword in keywords:
                    ...
                    try:
                        ...
                        if keyword == keywords[0] and url == urls[0]:
                            try:
                                ...
                                self.logger.info(
                                    "Saved page HTML to wellfound_debug.html for analysis"
                                )
                            except Exception as e:
                        ...
                        job_cards = (
                            soup.find_all("div", attrs={"data-test": "JobSearchResult"})
                            or soup.find_all(
                                "div", class_=lambda x: x and "styles_component" in str(x)
                            )
                            or soup.find_all(
                                "div", class_=lambda x: x and "job-listing" in str(x).lower()
                            )
                            or soup.find_all(
                                "div", class_=lambda x: x and "result" in str(x).lower()
                            )
                            or soup.find_all(
                                "a",
                                href=lambda x: x and "/company/" in str(x) and "/jobs/" in str(x),
                            )
                        )

                        self.logger.info(
                            f"Found {len(job_cards)} job cards on Wellfound from {url}"
                        )
                ...
                for card in job_cards[:max_results]:
                    try:
                        ...
                        if card.name == "a":
                            ...
                            if parent:
                                company_elem = parent.find(
                                    ["h2", "h3", "span"],
                                    class_=lambda x: x and "company" in str(x).lower(),
                                )
                                if company_elem:
                            ...
                            # Div-based card
                            title_elem = (
                                card.find("h2", class_=lambda x: x and "title" in str(x).lower())
                                or card.find("h3")
                                or card.find("a", class_=lambda x: x and "title" in str(x).lower())
                            )
                            if title_elem:
                                title = title_elem.get_text(strip=True)

                            company_elem = (
                                card.find("h2", class_=lambda x: x and "company" in str(x).lower())
                                or card.find("span", attrs={"data-test": "StartupResult-name"})
                                or card.find(
                                    "div", class_=lambda x: x and "company" in str(x).lower()
                                )
                            )
                            if company_elem:
                                company = company_elem.get_text(strip=True)
                            ...
                            location_elem = card.find(
                                ...
                                string=lambda x: x
                                and any(
                                    loc in str(x) for loc in ["France", "Paris", "Lyon", "Remote"]
                                ),
                            )
                            if location_elem:
                        ...
                        is_internship = any(
                            term in title.lower()
                            for term in ["intern", "stage", "apprentice", "graduate"]
                        )

                        # Only include France jobs
                        if (
                            "france" in job_location.lower()
                            or "paris" in job_location.lower()
                            or "french" in job_location.lower()
                        ):
                            job = JobOpportunity(
                                title=title,
                                company=company,
...
class JobHunter:
        ...
        for source in sources:
            try:
                if source.lower() == "remotive":
                    ...
                    jobs = self.scraper.scrape_welcometothejungle(
                        keywords, location, max_per_source
                    )
                    all_jobs.extend(jobs)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "job_types": dict(pd.Series([job.job_type for job in self.jobs_db]).value_counts())
            if self.jobs_db
            else {},
            "locations": dict(
                pd.Series([job.location for job in self.jobs_db]).value_counts().head(10)
            )
            if self.jobs_db
            else {},
            "recent_jobs": len(
                [
                    ...
                    if (datetime.now() - datetime.fromisoformat(j.posted_date)).days <= 7
                ]
            )
            if self.jobs_db
            else 0,
