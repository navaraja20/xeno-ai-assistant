...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QCheckBox, QSpinBox, QProgressBar, QTabWidget, QGroupBox,
    QFileDialog, QMessageBox, QHeaderView
)
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QTimer
from PyQt6.QtGui import QFont

from src.jobs.job_hunter import get_job_hunter, JobOpportunity
    ...
    def run(self):
        try:
            ...
            for idx, job_idx in enumerate(self.job_indices):
                self.progress.emit(
                    int((idx / total) * 100),
                    f"Creating application {idx + 1}/{total}..."
                )
...
class JobHunterWidget(QWidget):
    ...
    def create_search_tab(self) -> QWidget:
        ...
        search_btn.setStyleSheet("background-color: #4CAF50; color: white; font-size: 14px; padding: 10px;")
    ...
    def create_applications_tab(self) -> QWidget:
        ...
        create_btn.setStyleSheet("background-color: #2196F3; color: white; font-size: 14px; padding: 10px;")
    ...
    def create_applications(self):
        ...
        if not self.hunter.resume_text:
            QMessageBox.warning(
                self,
                "No Resume",
                "Please load your resume first in the Search tab"
            )
            return
