...
class XENOBrowserServer:
    ...
    async def handle_send_email(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(self.desktop_client, {"type": "send_email", "data": email_data})

            # Send confirmation back to browser
            await self.send_message(websocket, {"type": "email_sent", "status": "success"})
        else:
            await self.send_error(websocket, "Desktop app not connected")

    async def handle_create_calendar(
        ...
        if self.desktop_client:
            await self.send_message(
                self.desktop_client, {"type": "create_calendar_event", "data": event_data}
            )

            await self.send_message(websocket, {"type": "calendar_created", "status": "success"})
        else:
            await self.send_error(websocket, "Desktop app not connected")
    ...
    async def handle_create_task(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        # Forward to desktop app
        if self.desktop_client:
            await self.send_message(self.desktop_client, {"type": "create_task", "data": task_data})

            await self.send_message(websocket, {"type": "task_created", "status": "success"})
        else:
            await self.send_error(websocket, "Desktop app not connected")
    ...
    async def handle_voice_command(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(
                self.desktop_client, {"type": "voice_command", "data": voice_data}
            )
        else:
            await self.send_error(websocket, "Desktop app not connected")
    ...
    async def handle_save_content(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(
                self.desktop_client, {"type": "save_content", "data": content_data}
            )
        else:
            await self.send_error(websocket, "Desktop app not connected")
    ...
    async def handle_sync_activity(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(
                self.desktop_client, {"type": "sync_activity", "activities": activities}
            )
    ...
    async def handle_open_dashboard(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(self.desktop_client, {"type": "show_window"})

    async def handle_predict_priority(
        self, websocket: WebSocketServerProtocol, data: Dict[str, Any]
    ):
        ...
        if self.desktop_client:
            ...
            await self.send_message(
                websocket, {"type": "priority_prediction", "priority": "medium"}
            )
    ...
    async def handle_ai_rewrite(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(
                self.desktop_client,
                {"type": "ai_rewrite_email", "data": rewrite_data, "reply_to": id(websocket)},
            )
        else:
            await self.send_error(websocket, "Desktop app not connected")
    ...
    async def handle_analyze_repo(self, websocket: WebSocketServerProtocol, data: Dict[str, Any]):
        ...
        if self.desktop_client:
            await self.send_message(
                self.desktop_client, {"type": "analyze_repository", "data": repo_data}
            )
    ...
    async def send_error(self, websocket: WebSocketServerProtocol, error: str):
        ...
        await self.send_message(
            websocket, {"type": "error", "error": error, "timestamp": datetime.now().isoformat()}
        )
    ...
    async def notify_activity_update(self):
        ...
        await self.broadcast({"type": "activity_update", "timestamp": datetime.now().isoformat()})
