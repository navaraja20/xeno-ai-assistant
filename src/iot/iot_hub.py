    ...
    async def sync_data(self) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"{self.base_url}/user/-/activities/date/today.json", headers=headers
            ) as response:
...
class GarminIntegration(WearableIntegration):
    ...
    async def sync_data(self) -> Dict[str, Any]:
        ...
        return {"steps": 9500, "heart_rate": 68, "stress": 35, "body_battery": 75}
...
class SmartPlugIntegration(SmartHomeDevice):
    ...
    async def get_state(self) -> Dict[str, Any]:
        ...
        return {"on": True, "power": 45.2, "voltage": 120.0, "current": 0.38}  # Watts
...
class IoTHub:
    ...
    async def send_wearable_notification(self, device_id: str, title: str, message: str) -> bool:
        """Send notification to wearable"""
        integration = self.integrations.get(device_id)
        if isinstance(integration, WearableIntegration):
    ...
    def get_health_summary(self, user_id: str, days: int = 7) -> Dict[str, Any]:
        ...
        user_metrics = [
            m for m in self.health_data if m.user_id == user_id and m.timestamp >= cutoff
        ]
