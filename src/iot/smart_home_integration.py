...
class Scene:
    ...
    async def execute(self):
        ...
        for action in self.actions:
            ...
            if device:
                tasks.append(
                    device.execute_command(action["command"], **action["kwargs"])
                )
    ...
    async def discover_devices(self) -> List[SmartDevice]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.base_url}/enterprises/project-id/devices",
                headers=headers
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    for device_data in data.get("devices", []):
                        ...
                        if "thermostat" in device_type.lower():
                            device = SmartThermostat(
                                ...
                                name=device_data.get("traits", {}).get("Info", {}).get("customName", "Nest"),
                                api_endpoint=self.base_url,
