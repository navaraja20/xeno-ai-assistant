...
class NotificationManager:
    ...
    def notify_linkedin(self, event_type: str, details: str):
        ...
        Args:
            ...
            details: Event details
        """
        self.notify(title=f"LinkedIn - {event_type}", message=details, urgency="normal", timeout=10)
    ...
    def notify_urgent(self, title: str, message: str):
        ...
        """
        self.notify(
            title=title, message=message, urgency="critical", timeout=0  # Stay until dismissed
        )

    def _add_to_history(self, title: str, message: str, urgency: str):
        """Add notification to history"""
        self.notification_history.append(
            {"title": title, "message": message, "urgency": urgency, "timestamp": datetime.now()}
        )
    ...
    def _check_emails(self):
        ...
        try:
            ...
            for email in emails:
                ...
                is_urgent = any(
                    word in subject.lower() for word in ["urgent", "important", "asap", "critical"]
                )

                if is_urgent:
                    self.notification_manager.notify_urgent(
                        f"URGENT Email from {sender}", f"{subject}\n{preview}"
                    )
                else:
