...
class LinkedInAutomation:
    ...
    def search_jobs(self, keywords: str, location: str = '', job_type: str = '', 
                   experience_level: str = '', remote: bool = False, max_results: int = 25) -> List[Dict]:
        """
        ...
        try:
            ...
            for card in job_cards[:max_results]:
                try:
                    ...
                    # Get job title
                    title_elem = self.page.query_selector('.job-details-jobs-unified-top-card__job-title')
                    if not title_elem:
                    ...
                    # Get company
                    company_elem = self.page.query_selector('.job-details-jobs-unified-top-card__company-name')
                    if not company_elem:
                        company_elem = card.query_selector('.job-card-container__company-name')
                    company = company_elem.inner_text().strip() if company_elem else 'Unknown'
                    
                    # Get location
                    location_elem = self.page.query_selector('.job-details-jobs-unified-top-card__bullet')
                    ...
                    salary_elem = self.page.query_selector('.job-details-jobs-unified-top-card__job-insight')
    ...
    def apply_to_job(self, job_url: str, cover_letter: str = None, 
                    phone: str = None, resume_path: str = None) -> bool:
        """
        ...
        try:
            ...
            for step in range(max_steps):
                try:
                    ...
                    text_inputs = self.page.query_selector_all('input[required]:not([type="file"]):not([type="checkbox"]):not([type="radio"])')
                    for inp in text_inputs:
                        if not inp.input_value():
                            ...
                            if 'experience' in label.lower():
                                ...
                                inp.fill('I am very interested in this position and believe my skills align well with the requirements.')
                    ...
                    if not next_btn:
                        next_btn = self.page.query_selector('button[aria-label*="Submit application"]')
                    if not next_btn:
                    ...
                    if next_btn:
                        ...
                        success_message = self.page.query_selector('h3:has-text("Application sent")')
                        if not success_message:
                            success_message = self.page.query_selector('h2:has-text("Your application was sent")')
