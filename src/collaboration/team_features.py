...
class TeamManager:
    ...
    def create_team(self, team_id: str, name: str, description: str, owner: str) -> Team:
        """Create a new team"""
        team = Team(
            team_id=team_id, name=name, description=description, owner=owner, members=[owner]
        )
    ...
    def get_user_teams(self, username: str) -> List[Team]:
        ...
        return [team for team in self.teams.values() if username in team.members]
    ...
    def update_team_settings(self, team_id: str, settings: Dict[str, Any], updated_by: str) -> bool:
...
class SharedCalendarManager:
    ...
    def create_calendar(
        self, calendar_id: str, team_id: str, name: str, description: str, color: str = "#5865f2"
    ) -> SharedCalendar:
    ...
    def add_event(self, calendar_id: str, event: Dict[str, Any], created_by: str) -> bool:
    ...
    def get_team_calendars(self, team_id: str) -> List[SharedCalendar]:
        ...
        return [cal for cal in self.calendars.values() if cal.team_id == team_id]

    def set_permission(self, calendar_id: str, username: str, permission: str) -> bool:
...
class TaskDelegationManager:
    ...
    def get_user_tasks(self, username: str, status: Optional[str] = None) -> List[TaskAssignment]:
        """Get tasks assigned to user"""
        tasks = [task for task in self.tasks.values() if task.assigned_to == username]
    ...
    def get_team_tasks(self, team_id: str, status: Optional[str] = None) -> List[TaskAssignment]:
        """Get all tasks for team"""
        tasks = [task for task in self.tasks.values() if task.team_id == team_id]
...
class TeamAnalytics:
    ...
    def __init__(self, team_manager: TeamManager, task_manager: TaskDelegationManager):
