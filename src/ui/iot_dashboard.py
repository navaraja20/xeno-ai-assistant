    ...
    def create_scenes_tab(self) -> QWidget:
        ...
        scenes = [
            (
                "🌅 Good Morning",
                "good_morning",
                "Turn on lights, open blinds, set thermostat to 72°F",
            ),
            ...
            (
                "🚗 Leaving",
                "leaving",
                "Turn off lights, lock doors, set away mode, lower thermostat",
            ),
        ...
        if "battery" in data:
            ...
            if battery > 50:
                self.battery_progress.setStyleSheet(
                    "QProgressBar::chunk { background-color: #57f287; }"
                )
            elif battery > 20:
                self.battery_progress.setStyleSheet(
                    "QProgressBar::chunk { background-color: #faa61a; }"
                )
            else:
                self.battery_progress.setStyleSheet(
                    "QProgressBar::chunk { background-color: #ed4245; }"
                )
