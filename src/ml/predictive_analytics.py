...
class PredictiveAnalytics:
    ...
    def predict_optimal_work_time(self, task_type: str) -> Tuple[datetime, float]:
        ...
        try:
            ...
            self.logger.info(
                f"Optimal time for {task_type}: {best_time.strftime('%A, %I:%M %p')} (score: {best_score:.2f})"
            )
    ...
    def _calculate_salary_fit(self, salary_range: str) -> float:
        ...
        if any(
            word in salary_range.lower() for word in ["competitive", "above market", "150k", "200k"]
        ):
            return 1.0
        return 0.6
    ...
    def _get_historical_productivity(self, day_of_week: int, hour: int, task_type: str) -> float:
        ...
        matching = [
            p
            for p in self.productivity_data
    ...
    def _load_training_data(self):
        ...
        try:
            ...
            if data_file.exists():
                ...
                self.logger.info(
                    f"Loaded training data: {len(self.job_training_data)} jobs, "
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.job_training_data:
            success_rate = sum(1 for d in self.job_training_data if d["got_interview"]) / len(
                self.job_training_data
            )
            if success_rate < 0.2:
                recommendations.append(
                    "Consider refining your job search criteria - low interview rate detected"
                )
        ...
        if self.email_training_data:
            ...
            if quick_replies > len(self.email_training_data) * 0.7:
                recommendations.append(
                    "You respond quickly to emails - consider scheduling specific email times"
                )

        # Productivity recommendations
        if self.productivity_data:
            morning_prod = np.mean(
                [p["productivity_score"] for p in self.productivity_data if 6 <= p["hour"] <= 12]
            )
            afternoon_prod = np.mean(
                [p["productivity_score"] for p in self.productivity_data if 13 <= p["hour"] <= 18]
            )

            if morning_prod > afternoon_prod * 1.2:
                recommendations.append(
                    "You're most productive in mornings - schedule important tasks before noon"
                )
            elif afternoon_prod > morning_prod * 1.2:
                recommendations.append(
                    "You're most productive in afternoons - save complex tasks for after lunch"
                )
