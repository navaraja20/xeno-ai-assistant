...
def test_iot_with_voice_control():
    ...
    try:
        ...
        light = smart_home.add_light(
            "living_room", "Living Room Light", "philips_hue"
        )  # Fixed: use add_light method
...
def test_federated_learning_with_privacy():
    ...
    try:
        ...
        base_params = {"layer1": np.random.randn(10, 10), "layer2": np.random.randn(10, 5)}

        model_id = manager.create_model(
            model_id="privacy_model_v1", base_params=base_params, privacy_enabled=True
        )
