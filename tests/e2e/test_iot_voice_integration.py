...
def test_e2e_device_grouping_workflow(smart_home_hub):
    ...
    # Create device group
    smart_home_hub.create_group("living_room", ["light_0", "light_1", "light_2"])
...
def test_e2e_emotion_detection_workflow():
    ...
    for text, acceptable_emotions in emotions:
        ...
        assert (
            emotion.name in acceptable_emotions
        ), f"Expected one of {acceptable_emotions}, got {emotion.name}"
...
def test_e2e_voice_controlled_smart_home(smart_home_hub, voice_engine):
    ...
    living_room_light = SmartLight("lr_light", "Living Room Light", "http://test.local", "key")
    thermostat = SmartThermostat("main_thermo", "Main Thermostat", "http://test.local", "key")
...
def test_e2e_emergency_response_workflow(smart_home_hub, voice_engine):
    ...
    automation = Automation(
        name="Emergency Mode", description="Emergency response", hub=smart_home_hub
    )
...
def test_e2e_daily_routine_automation(smart_home_hub, voice_engine):
    ...
    from src.iot.smart_home_integration import Automation, Scene
...
def test_e2e_smart_home_security_system(smart_home_hub):
    ...
    intrusion = Automation(
        name="Intrusion Detected", description="Security alert", hub=smart_home_hub
    )
